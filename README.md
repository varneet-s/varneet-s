<h1 align="center">Hey, I'm Varneet 👋</h1>
 
<p align="center">
  <b>Business Operations Analyst → Aspiring Business Analyst</b><br/>
  I find patterns in messy data, map broken processes, and structure information so people can act on it.
</p>

<p align="center">
  <a href="https://www.linkedin.com/in/varneet-singh/">
    <img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" />
  </a>
  <a href="https://varneet-s.github.io/varneet-portfolio/">
    <img src="https://img.shields.io/badge/Portfolio-111827?style=for-the-badge&logo=githubpages&logoColor=white" />
  </a>
  <img src="https://komarev.com/ghpvc/?username=varneet-s&style=for-the-badge&color=0A66C2" />
</p>
 
## 🙋 About Me
 
- 📍 Ludhiana, Punjab, India
- 💼 Currently: **Business Operations Analyst** at I.P. Pharma — building tracking systems, analysing revenue constraints, and documenting operational processes
- 🎯 Targeting: **Business Analyst** roles in data-heavy, ops-focused environments
- 🛠️ I work in Excel, SQL, and Tableau — no fluff, no Python (yet)
- 🚧 Building: a SQL-based business querying project — up next
- 📚 Certified via **Microsoft BA Professional Certificate** & **Deloitte Data Analytics (Forage)**
---
 
## 🧰 Tools & Skills
 
**Analysis & BA**
 
![Excel](https://img.shields.io/badge/Excel-217346?style=flat-square&logo=microsoftexcel&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-4479A1?style=flat-square&logo=mysql&logoColor=white)
![Tableau](https://img.shields.io/badge/Tableau-E97627?style=flat-square&logo=tableau&logoColor=white)
![Google Sheets](https://img.shields.io/badge/Google_Sheets-34A853?style=flat-square&logo=googlesheets&logoColor=white)
![Visio](https://img.shields.io/badge/Microsoft_Visio-3955A3?style=flat-square&logo=microsoftvisio&logoColor=white)
 
**Frameworks & Methods**
 
`Requirements Gathering` · `Gap Analysis` · `Process Mapping` · `Stakeholder Management` · `KPI Tracking` · `Data Cleaning` · `Trend Analysis` · `Customer Segmentation`
 
---
 
## 📂 Projects

### 📦 [Olist E-Commerce Operations Analysis](https://github.com/varneet-s/olist-analysis)
**Tools:** Excel · Tableau · Draw.io  
End-to-end BA project on Brazil's largest e-commerce dataset — stakeholder mapping, process analysis, gap identification, and recommendations.

---

### 💊 [I.P. Pharma Operational Dashboard & Sales Tracking](https://varneet-s.github.io/varneet-portfolio/projects/pharma.html)
**Tools:** Excel · Operations Management  
Built a structured stock-tracking system to replace diary-based records across 30-40 products, cutting reporting lag from 3 days to real-time.
 
---
 
### 🛒 [Retail Sales & Customer Behaviour Analysis](https://github.com/varneet-s/Vrinda-Store-Data-Analysis)
**Tools:** Excel  
Cleaned and transformed raw retail data to identify revenue patterns across channels, regions, and customer segments.
 
- Women (~65%) and the 30–49 age group (~50%) are primary revenue contributors
- Amazon, Flipkart, and Myntra account for ~80% of total sales
- Built an Excel dashboard visualising KPIs to support targeted marketing and channel strategy
---
 
## 📜 Certifications
 
| Certification | Issuer |
|---|---|
| Business Analyst Professional Certificate | Microsoft |
| Data Analytics Job Simulation | Deloitte × Forage |
 
---
 
<p align="center">
  <i>Open to BA roles — full-time, contract, or internship. Drop a message on <a href="https://www.linkedin.com/in/varneet-singh/">LinkedIn</a>.</i>
</p>
